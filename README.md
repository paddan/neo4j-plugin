...
2) If you download the Actions artifact: there is a zip file `neo4j-plugin-1.0.9.zip` inside the downloaded `neo4j-plugin.zip` — install that one.
3) In IntelliJ IDEA: `Settings/Preferences > Plugins > ⚙ > Install Plugin from Disk...`.
4) Select the plugin ZIP (e.g., `build/distributions/neo4j-plugin-1.0.9.zip` or the downloaded release ZIP), install, and restart the IDE.
