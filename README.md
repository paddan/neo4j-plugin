...
2) If you download the Actions artifact: There's a zip-file `neo4j-plugin-0.1.0.zip` inside the downloaded `neo4j-plugin.zip`, install that as a plugin in intellij.
3) In IntelliJ IDEA: `Settings/Preferences > Plugins > ⚙ > Install Plugin from Disk...`.
4) Select the plugin ZIP (e.g., `build/distributions/neo4j-plugin-0.1.0.zip` or the extracted one from `neo4j-plugin.zip`), install, and restart the IDE.
